...
import { ServiceReviewForm } from './ReviewForm';
...
export function ServiceReviewsSection({ serviceId, serviceOrderId }: ServiceReviewsSectionProps) {
  ...
  const { data: ratingSummary, isLoading: summaryLoading } = useQuery({
    ...
    queryFn: () => reviewService.getServiceRatingSummary(serviceId),
  });

  const { data: reviewsData, isLoading: reviewsLoading } = useQuery({
    queryKey: ['service-reviews', serviceId, page],
    queryFn: () => reviewService.getServiceReviews(serviceId, page, 5),
  });

  const { data: canReview } = useQuery({
    queryKey: ['can-review-service', serviceId],
    queryFn: () => reviewService.canReviewService(serviceId),
