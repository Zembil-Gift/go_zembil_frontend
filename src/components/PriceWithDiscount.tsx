...
export const PriceWithDiscount: React.FC<PriceWithDiscountProps> = ({
  ...
  if (!hasDiscount) {
    return (
      <div className={`${classes.discounted} ${theme === 'onRed' ? 'text-white' : 'text-gray-900 dark:text-white'}`}>
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-3">
        <div className={`${classes.discounted} ${theme === 'onRed' ? 'text-white' : 'text-red-600 dark:text-red-400'}`}>
          {getCurrencySymbol(currency)}{discountedPrice.toFixed(decimals)}
        </div>
        <div className={`${classes.original} ${theme === 'onRed' ? 'text-white/80' : 'text-gray-500 dark:text-gray-400'} line-through`}>
          {getCurrencySymbol(currency)}{originalPrice.toFixed(decimals)}
        </div>
      </div>
      {showSavings && savings > 0 && (
        <div className={`${classes.savings} ${theme === 'onRed' ? 'text-white/90' : 'text-green-600 dark:text-green-400'} font-medium flex flex-wrap items-center gap-1`}>
          ...
          {discount.discountType === 'PERCENTAGE' && discount.discountPercentage && (
            <span className={`${theme === 'onRed' ? 'text-white/80' : 'text-gray-500 dark:text-gray-400'} text-xs`}>
