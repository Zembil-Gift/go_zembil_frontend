...
class ServiceOrderService {
  ...
  async createOrder(
    ...
    >("/api/service-orders", {
      ...request,
      scheduledDateTime:
        toInstantISOString(request.scheduledDateTime) ||
  ...
  async getCustomerOrders(
    page: number = 0,
    size: number = 10
  ): Promise<PagedServiceOrderResponse> {
  ...
  async confirmOrder(orderId: number): Promise<ServiceOrderResponse> {
    return await apiService.postRequest<ServiceOrderResponse>(
      `/api/service-orders/${orderId}/confirm`,
      {}
    );
  }
  ...
  async markInProgress(orderId: number): Promise<ServiceOrderResponse> {
    return await apiService.postRequest<ServiceOrderResponse>(
      `/api/service-orders/${orderId}/in-progress`,
      {}
    );
  }
  ...
  async markNoShow(orderId: number): Promise<ServiceOrderResponse> {
    return await apiService.postRequest<ServiceOrderResponse>(
      `/api/service-orders/${orderId}/no-show`,
      {}
    );
  }
  ...
  async vendorCancelOrder(
    orderId: number,
    reason: string,
    validReason: boolean = false
  ): Promise<ServiceOrderResponse> {
    const url = `/api/service-orders/${orderId}/vendor-cancel?reason=${encodeURIComponent(
      reason
   ...
   */
  async vendorRescheduleOrder(
    orderId: number,
    newDateTime: string
  ): Promise<ServiceOrderResponse> {
    const instant = toInstantISOString(newDateTime) || newDateTime;
    const url = `/api/service-orders/${orderId}/vendor-reschedule?newDateTime=${encodeURIComponent(
      instant
    )}`;
    return await apiService.postRequest<ServiceOrderResponse>(url, {});
  }
  ...
  async cancelOrder(
    ...
    const url = reason
      ? `/api/service-orders/${orderId}/cancel?reason=${encodeURIComponent(
          reason
        )}`
  ...
  async rescheduleOrder(
    ...
    const instant = toInstantISOString(newDateTime) || newDateTime;
    const url = `/api/service-orders/${orderId}/reschedule?newDateTime=${encodeURIComponent(
      instant
    )}`;
    return await apiService.postRequest<ServiceOrderResponse>(url, {});
  }
   ...
   */
  getStatusDisplay(status: ServiceOrderStatus): {
    text: string;
    color: string;
    bgColor: string;
  } {
   ...
   */
  getPaymentStatusDisplay(status: PaymentStatus): {
    text: string;
    color: string;
    bgColor: string;
  } {
  ...
  canVendorCancel(order: ServiceOrderResponse): boolean {
    return (
      ["BOOKED", "CONFIRMED_BY_VENDOR"].includes(order.status) &&
