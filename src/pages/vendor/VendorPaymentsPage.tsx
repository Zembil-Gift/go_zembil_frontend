...
export default function VendorPaymentsPage() {
  ...
  const { data: payoutSummary } = useQuery<VendorPayoutSummary>({
    queryKey: ["vendor-payout-summary"],
    queryFn: () => vendorService.getPayoutSummary(),
    enabled: isAuthenticated && isVendor,
  });
  ...
  const {
    ...
    isFetching: isPayoutHistoryFetching,
  } = useQuery({
    queryKey: ["vendor-payout-history", payoutPage, pageSize, sortDirection],
    queryFn: () =>
      vendorService.getPayoutHistory(
        payoutPage,
        pageSize,
        `createdAt,${sortDirection}`
      ),
  ...
  const refreshProcessingMutation = useMutation({
  ...
  const canRefreshProcessingPayout = (item: VendorPayoutHistoryItem) => {
    const isRefreshableStatus =
      item.status === "PROCESSING" || item.status === "IN_TRANSIT";
    const isChapaPayout =
      item.payoutMethod === "CHAPA_TRANSFER" ||
      item.payoutMethod === "CHAPA_SUBACCOUNT";
    return isRefreshableStatus && isChapaPayout;
  };

  const canRequestPayout = payoutOverview?.requestPayout?.allowed ?? false;

  const filteredHistory = useMemo(() => {
    const rows = payoutHistory?.content || [];
  ...
  }, [payoutHistory?.content, statusFilter, startDateFilter, endDateFilter]);

  const currentPageNumber = payoutHistory?.number ?? payoutPage;
  const totalHistoryPages = Math.max(payoutHistory?.totalPages ?? 1, 1);
  const totalPayoutElements = payoutHistory?.totalElements ?? 0;
  const effectiveSummary = payoutSummary || payoutOverview?.summary;
        ...
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            ...
            disabled={
              ...
              refreshProcessingMutation.isPending
            }
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 ${
                isPayoutOverviewFetching || isPayoutHistoryFetching
                  ? "animate-spin"
                  : ""
              }`}
            />
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-lg border p-4">
              ...
              <p className="mt-1 text-sm font-medium">
                {formatDateTime(
                  payoutOverview?.payoutPolicy?.nextEligiblePayoutAt
                )}
              </p>
              <p className="mt-2 text-xs text-muted-foreground">
                      ...
                      <TableRow key={item.payoutId}>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            ...
                            {canRefreshProcessingPayout(item) && (
                              <Button
                                variant="outline"
                                size="sm"
                                className="mt-2"
                                onClick={() =>
                                  refreshProcessingMutation.mutate(
                                    item.payoutId
                                  )
                                }
                                disabled={refreshProcessingMutation.isPending}
                              >
