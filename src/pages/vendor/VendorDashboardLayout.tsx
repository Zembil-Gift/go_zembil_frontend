...
export default function VendorDashboardLayout() {
  ...
  const { data: packagesData } = useQuery({
    queryKey: ["vendor", "packages", "badge-count"],
  ...
  const { data: packageOrdersData } = useQuery({
    queryKey: ["vendor", "package-orders", "badge-count"],
    queryFn: async () => {
  ...
  const customOrdersNeedsAction = customOrders.filter(
    (o) => o.status === "SUBMITTED" || o.status === "PAID"
  ).length;

  const productOrders = productOrdersData?.content || [];
  const productOrdersNeedsAction = productOrders.filter(
    (o: any) => o.status === "PENDING" || o.status === "CONFIRMED"
  ).length;

  const packagesCount = packagesData?.totalElements || 0;

  const packageOrders = packageOrdersData || [];
  const packageOrdersNeedsAction = packageOrders.filter(
    (o: any) =>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <p className="text-gray-600 mb-4">
          {t("You need to be a vendor to access this dashboard.")}
        </p>
        <Button asChild>
  ...
  const navigationItems: NavigationItem[] = [
    ...
    },
    {
      ...
      path: "/vendor/product-orders",
      label: t("Product Orders"),
      icon: ShoppingBag,
      show:
        vendorProfile?.vendorType === "PRODUCT" ||
        vendorProfile?.vendorType === "HYBRID",
      badge: productOrdersNeedsAction,
    },
    {
      value: "packages",
    ...
    {
      value: "package-orders",
      path: "/vendor/package-orders",
      label: t("Package Orders"),
      icon: ShoppingBag,
      show:
        vendorProfile?.vendorType === "PRODUCT" ||
        vendorProfile?.vendorType === "HYBRID",
      badge: packageOrdersNeedsAction,
    },
    {
      value: "events",
      path: "/vendor/events",
    ...
    },
    {
      ...
      path: "/vendor/service-orders",
      label: t("Service Bookings"),
      icon: Calendar,
      show:
        vendorProfile?.vendorType === "SERVICE" ||
        vendorProfile?.vendorType === "HYBRID",
    ...
    {
      value: "custom-orders",
      path: "/vendor/custom-orders",
      label: t("Custom Orders"),
      icon: ShoppingBag,
      show:
        vendorProfile?.vendorType === "PRODUCT" ||
        vendorProfile?.vendorType === "HYBRID",
      badge: customOrdersNeedsAction,
    },
    {
      value: "check-in",
  ...
  const isCustomOrderDetailPage = location.pathname.startsWith(
    "/vendor/custom-orders/"
  );
  const normalizedVendorStatus = (vendorProfile?.status || "")
    .trim()
    .toUpperCase();
  ...
  return (
    <div
      className={cn(
        "min-h-screen",
        isCustomOrderDetailPage ? "bg-white" : "bg-gray-50"
      )}
    >
      {/* Mobile Header */}
        ...
        <div className="flex items-center justify-between h-full px-4">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              {isVendorApproved ? (
                <Badge className="bg-green-500 text-white text-[10px] h-4 px-1 w-fit">
                  {t("Approved")}
                </Badge>
              ) : isVendorRejected ? (
                <Badge className="bg-red-500 text-white text-[10px] h-4 px-1 w-fit">
                  {t("Rejected")}
                </Badge>
              ) : (
                <Badge className="bg-amber-500 text-white text-[10px] h-4 px-1 w-fit">
                  {t("Pending")}
                </Badge>
              )}
            </div>
              ...
              return (
                ...
                >
                  ...
                  {item.badge && item.badge > 0 && (
                    <Badge
                      variant="destructive"
                      className="ml-2 h-5 min-w-5 px-1.5 text-xs flex items-center justify-center"
                    >
                      {item.badge}
                    </Badge>
                  )}
          ...
          <div className="hidden lg:block p-4 border-t border-white/10">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="w-full justify-start gap-3 h-auto py-2 text-white hover:bg-white/10 hover:text-white"
                >
                  <Avatar className="h-8 w-8">
                  ...
                  {isVendorRejected ? (
                    <>
                      <p className="mt-1 text-sm text-red-700">
                        {t("Please update your business details and resubmit your application from Settings.")}
                      </p>
                      <div className="mt-3 rounded-md border border-red-200 bg-white p-3">
                        ...
                        {vendorProfile.rejectionReason ? (
                          ...
                          <p className="mt-1 text-sm text-red-800">
                            {t("No rejection reason provided.")}
                          </p>
                        )}
