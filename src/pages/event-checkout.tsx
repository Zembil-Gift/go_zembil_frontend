...
export default function EventCheckout() {
  ...
  const selectedTicketsFromState = location.state?.selectedTickets as
  ...
  useEffect(() => {
    if (
      ...
      !enabledPaymentMethods.includes(paymentProvider)
    ) {
      setPaymentProvider(
        enabledPaymentMethods[0] as "stripe" | "chapa" | "telebirr"
      );
    }
  ...
  const getTotalTickets = () => {
    return ticketSelections.reduce(
      (sum, sel) => sum + sel.recipients.length,
      0
    );
  };
  ...
  const getTotalPrice = () => {
    return ticketSelections.reduce(
      (sum, sel) => sum + sel.priceMinor * sel.recipients.length,
      0
    );
  };
  ...
  const updateRecipient = (
    ...
    field: keyof TicketPurchaseItem,
    value: string
  ) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderRequest: CreateEventOrderRequest = {
        ...
        paymentProvider: paymentProvider.toUpperCase() as
          | "STRIPE"
          | "CHAPA"
          | "TELEBIRR",
      };
                ...
                <CardContent>
                  {enabledPaymentMethods.length === 0 ? (
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        setPaymentProvider(
                          value as "stripe" | "chapa" | "telebirr"
                        )
                      }
                    >
                      ...
                      >
                        ...
                        <span className="font-bold text-eagle-green">
                          {eventOrderService.formatCurrency(
                            selection.priceMinor * selection.recipients.length,
                            selection.currency
                          )}
                        </span>
                      </div>
