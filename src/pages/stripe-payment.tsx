...
function PaymentForm({
  ...
  const normalizedOrderType = (orderType || "PRODUCT").toUpperCase();
  const serviceSuccessUrl = `/service-order-success?orderId=${orderId}`;
  const defaultSuccessUrl = `/payment-success?orderId=${orderId}&orderType=${normalizedOrderType}`;
  const returnUrl =
    normalizedOrderType === "SERVICE"
      ? `${window.location.origin}${serviceSuccessUrl}`
      : `${window.location.origin}${defaultSuccessUrl}`;
        ...
        if (paymentIntent.status === "succeeded") {
          ...
          setTimeout(() => {
            if (normalizedOrderType === "SERVICE") {
              window.location.href = serviceSuccessUrl;
              return;
            }

            window.location.href = `${defaultSuccessUrl}&payment_intent=${paymentIntent.id}`;
          }, 1500);
...
export default function StripePaymentPage() {
  ...
  const initializePayment = async (
    ...
    try {
      ...
      console.log(
        "🔄 Initializing Stripe payment for order:",
        orderIdNum,
        "type:",
        type
      );
      ...
      const billingDetails = billingAddressData
        ? {
            name:
              ...
              billingAddressData.contactName ||
              "",
  ...
  const handlePaymentSuccess = (paymentIntentId: string) => {
    ...
    console.log(
      "🔔 Stripe webhook will process the payment_intent.succeeded event"
    );
