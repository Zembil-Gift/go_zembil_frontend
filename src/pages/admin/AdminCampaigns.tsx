...
function parseEligibilityRulesToBuilder(
  json: string | null | undefined
): EligibilityBuilderState {
  ...
  try {
    ...
    if (parsed.registeredWithinCampaignWindow === true)
      next.registeredWithinCampaignWindow = true;

    if (
      Array.isArray(parsed.vendorCategoryIds) &&
      parsed.vendorCategoryIds.length > 0
    ) {
...
function buildEligibilityRulesObject(
  builder: EligibilityBuilderState,
  campaignType: CampaignType
): Record<string, unknown> {
  const rules: Record<string, unknown> = {};

  const parseEnabledNumber = (
    enabled: boolean,
    value: string
  ): number | null => {
  ...
  const minAccountAgeDays = parseEnabledNumber(
    builder.minAccountAgeDaysEnabled,
    builder.minAccountAgeDays
  );
  ...
  if (
    ...
    const minProductOrders = parseEnabledNumber(
      builder.minProductOrdersEnabled,
      builder.minProductOrders
    );
    if (minProductOrders !== null) rules.minProductOrders = minProductOrders;

    const minServiceBookings = parseEnabledNumber(
      builder.minServiceBookingsEnabled,
      builder.minServiceBookings
    );
    if (minServiceBookings !== null)
      rules.minServiceBookings = minServiceBookings;

    const minEventOrders = parseEnabledNumber(
      builder.minEventOrdersEnabled,
      builder.minEventOrders
    );
  ...
  if (campaignType === "VENDOR_PARTICIPATION") {
    ...
    const minCompletedProductOrders = parseEnabledNumber(
      builder.minCompletedProductOrdersEnabled,
      builder.minCompletedProductOrders
    );
    if (minCompletedProductOrders !== null)
      rules.minCompletedProductOrders = minCompletedProductOrders;

    const minPaidServiceOrders = parseEnabledNumber(
      builder.minPaidServiceOrdersEnabled,
      builder.minPaidServiceOrders
    );
    if (minPaidServiceOrders !== null)
      rules.minPaidServiceOrders = minPaidServiceOrders;

    const minPaidEventOrders = parseEnabledNumber(
      builder.minPaidEventOrdersEnabled,
      builder.minPaidEventOrders
    );
    if (minPaidEventOrders !== null)
      rules.minPaidEventOrders = minPaidEventOrders;

    const minPaidCustomOrders = parseEnabledNumber(
      builder.minPaidCustomOrdersEnabled,
      builder.minPaidCustomOrders
    );
    if (minPaidCustomOrders !== null)
      rules.minPaidCustomOrders = minPaidCustomOrders;

    const minTotalRevenueMinor = parseEnabledNumber(
      builder.minTotalRevenueMinorEnabled,
      builder.minTotalRevenueMinor
    );
    if (minTotalRevenueMinor !== null)
      rules.minTotalRevenueMinor = minTotalRevenueMinor;

    if (builder.registeredWithinCampaignWindow)
      rules.registeredWithinCampaignWindow = true;
  ...
  try {
    ...
    return {
      minimumSalesAmountMinor:
        typeof parsed.minimumSalesAmountMinor === "number"
          ? parsed.minimumSalesAmountMinor
          : undefined,
      minimumOrderCount:
        typeof parsed.minimumOrderCount === "number"
          ? parsed.minimumOrderCount
          : undefined,
    };
  } catch {
    return {};
...
export default function AdminCampaigns() {
  ...
  const [editingCampaign, setEditingCampaign] = useState<EventCampaign | null>(
    null
  );
  ...
  const [eligibilityBuilder, setEligibilityBuilder] =
    useState<EligibilityBuilderState>(DEFAULT_ELIGIBILITY_BUILDER);
      ...
      return subCategoryGroups
        .filter(
          (
            ...
            { id: number; name: string; parentCategoryName: string }[]
    ...
    mutationFn: async (data: EventCampaignRequest) => {
      ...
      if (pendingImageFiles.length > 0) {
        try {
          ...
          await campaignService.uploadCampaignImage(
            campaign.id,
            pendingImageFiles[0]
          );
        } catch (err: unknown) {
          ...
          toast({
            title: "Warning",
            description: "Campaign created but image upload failed: " + msg,
            variant: "destructive",
          });
        } finally {
    ...
    mutationFn: async ({
      ...
      if (pendingImageFiles.length > 0) {
        try {
          ...
          toast({
            title: "Warning",
            description: "Campaign updated but image upload failed: " + msg,
            variant: "destructive",
          });
        } finally {
          setIsUploadingImage(false);
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast({
        title: "Success",
        description: `Campaign ${result.active ? "activated" : "deactivated"}`,
      });
    },
    onError: (error: unknown) => {
      const msg =
        error instanceof Error ? error.message : "Failed to toggle campaign";
      toast({ title: "Error", description: msg, variant: "destructive" });
    },
  });
  ...
  function openEdit(campaign: EventCampaign) {
    ...
    setPendingImageFiles([]);
    setEligibilityBuilder(
      parseEligibilityRulesToBuilder(campaign.eligibilityRules)
    );
  ...
  async function validateStep(): Promise<boolean> {
    ...
    if (step === 2) {
      ...
      if (isVendorParticipation) {
        ...
        if (
          !actionType ||
          !VENDOR_ACTION_TYPES.includes(
            actionType as (typeof VENDOR_ACTION_TYPES)[number]
          )
        ) {
          form.setError("actionType", {
            message: "Select a valid vendor action type",
          });
          return false;
        }
        ...
        if (actionType === "COMPLETE_MIN_SALES") {
          ...
          if (!Number.isFinite(value) || value <= 0) {
            form.setError("minimumSalesAmountMinor", {
              message: "Minimum sales amount must be greater than 0",
            });
            return false;
          }
        }
        ...
        if (actionType === "COMPLETE_MIN_ORDERS") {
          ...
          if (!Number.isFinite(value) || value <= 0) {
            form.setError("minimumOrderCount", {
              message: "Minimum order count must be greater than 0",
            });
            return false;
          }
        }
    ...
    if (step === 3) {
      ...
      if (isVendorParticipation) {
        ...
        if (
          (rewardType === "COMMISSION_BONUS" || rewardType === "FIXED_BONUS") &&
          (!Number.isFinite(rewardValue) || rewardValue <= 0)
        ) {
          form.setError("rewardValue", {
            message: "Reward value must be greater than 0 for this reward type",
          });
          return false;
        }
        ...
        if (
          ...
          (!Number.isFinite(rewardDurationDays) || rewardDurationDays <= 0)
        ) {
    ...
    if (step === 4) {
      if (isUserParticipation) {
        ...
        for (const [flag, valueKey] of toValidate) {
          ...
          if (!Number.isFinite(parsed) || parsed < 0) {
            toast({
              title: "Invalid eligibility value",
              description:
                "Please provide valid non-negative numbers for selected user rules.",
              variant: "destructive",
            });
            return false;
          }
        ...
        for (const [flag, valueKey] of toValidate) {
          ...
          if (!Number.isFinite(parsed) || parsed < 0) {
            toast({
              title: "Invalid eligibility value",
              description:
                "Please provide valid non-negative numbers for selected vendor rules.",
              variant: "destructive",
            });
            return false;
          }
        ...
        if (eligibilityBuilder.vendorCategoryIdsEnabled) {
          ...
          if (parsedIds.length === 0) {
            toast({
              ...
              description:
                "Please add at least one valid category ID when Vendor Category IDs is selected.",
              variant: "destructive",
            });
            return false;
          }
        ...
        if (eligibilityBuilder.vendorCitiesEnabled) {
          ...
          if (cities.length === 0) {
            toast({
              ...
              description:
                "Please add at least one city when Vendor Cities is selected.",
              variant: "destructive",
            });
            return false;
          }
      ...
      if (eligibilityBuilder.minAccountAgeDaysEnabled) {
        ...
        if (!Number.isFinite(parsed) || parsed < 0) {
          toast({
            title: "Invalid account age",
            description:
              "Please provide a valid non-negative minimum account age.",
            variant: "destructive",
          });
          return false;
        }
  ...
  function buildPayload(values: CampaignFormValues): EventCampaignRequest {
    ...
    if (values.campaignType === "VENDOR_PARTICIPATION") {
      ...
      return {
        ...
        rewardValue:
          values.rewardType === "FEATURED_PLACEMENT"
            ? null
            : values.rewardValue ?? null,
        rewardDurationType: (values.rewardDurationType ||
          null) as EventCampaignRequest["rewardDurationType"],
        rewardDurationDays:
          values.rewardDurationType === "FIXED_PERIOD"
            ? values.rewardDurationDays ?? null
            : null,
        eligibilityRules:
          Object.keys(generatedEligibilityRules).length > 0
            ? JSON.stringify(generatedEligibilityRules)
            : null,
      };
    }
    ...
    >
      <div className="space-y-6 p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          <Button
            onClick={openCreate}
            className="bg-eagle-green hover:bg-eagle-green/90 whitespace-nowrap text-white"
          >
                      ...
                      <TableRow key={campaign.id}>
                        ...
                        <TableCell>
                          <Switch
                            ...
                            onCheckedChange={() =>
                              toggleMutation.mutate(campaign.id)
                            }
                            disabled={toggleMutation.isPending}
                          />
                        </TableCell>
                        <TableCell>
                          {PARTICIPATION_TYPES.includes(
                            ...
                            <span className="inline-flex items-center gap-1 text-sm">
                              ...
                              <span className="text-muted-foreground text-xs">
                                ({campaign.approvedCount ?? 0} approved /{" "}
                                {campaign.pendingCount ?? 0} pending)
                              </span>
                            </span>
                          ) : (
                        ...
                        <div className="relative rounded-lg overflow-hidden border bg-muted h-44 group">
                          ...
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  await campaignService.deleteCampaignImage(
                                    editingCampaign.id
                                  );
                                  setEditingCampaign({
                                    ...editingCampaign,
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Higher number = higher priority in carousel
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-center justify-between rounded-lg border p-4">
                        <div className="space-y-0.5">
                          ...
                          <FormDescription>
                            Campaign will be visible when active and within the
                            date range
                          </FormDescription>
                        </div>
                        <FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                                ...
                                (sub: {
                                  ...
                                  >
                                    {sub.parentCategoryName
                                      ? `${sub.parentCategoryName} • ${sub.name}`
                                      : sub.name}
                                  </SelectItem>
                                )
                              )}
                          ...
                          <FormDescription>
                            Users will be directed to this subcategory when they
                            click the CTA
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                >
                                  ...
                                  <SelectContent>
                                    <SelectItem value="COMPLETE_MIN_SALES">
                                      {ACTION_TYPE_LABELS.COMPLETE_MIN_SALES}
                                    </SelectItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            {(
                              Object.keys(
                                REWARD_DURATION_LABELS
                              ) as (keyof typeof REWARD_DURATION_LABELS)[]
                            ).map((d) => (
                    ...
                    <div className="flex items-center gap-3">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setEligibilityBuilder((prev) => ({
                            ...prev,
                            minAccountAgeDaysEnabled: checked === true,
                          }))
                        }
                      />
                      <label
                        htmlFor="rule-minAccountAgeDays"
                  ...
                  {isUserParticipation && (
                    <div className="rounded-lg border p-4 space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <label htmlFor="rule-emailVerified" className="text-sm">
                          Email verified
                        </label>
                      </div>
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minProductOrdersEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minProductOrders"
                            className="text-sm"
                          >
                        ...
                        {eligibilityBuilder.minProductOrdersEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minProductOrders: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minServiceBookingsEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minServiceBookings"
                            className="text-sm"
                          >
                            Minimum service bookings
                          </label>
                        </div>
                        {eligibilityBuilder.minServiceBookingsEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minServiceBookings: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minEventOrdersEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minEventOrders"
                            className="text-sm"
                          >
                            Minimum event orders
                          </label>
                        </div>
                    ...
                    <div className="rounded-lg border p-4 space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setEligibilityBuilder((prev) => ({
                              ...prev,
                              vendorApproved: checked === true,
                            }))
                          }
                        />
                        <label
                          htmlFor="rule-vendorApproved"
                          className="text-sm"
                        >
                          Vendor approved
                        </label>
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minCompletedProductOrdersEnabled:
                                  checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minCompletedProductOrders"
                            className="text-sm"
                          >
                            Min completed product orders
                          </label>
                        </div>
                        {eligibilityBuilder.minCompletedProductOrdersEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minCompletedProductOrders: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidServiceOrdersEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minPaidServiceOrders"
                            className="text-sm"
                          >
                            Min paid service orders
                          </label>
                        </div>
                        {eligibilityBuilder.minPaidServiceOrdersEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidServiceOrders: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidEventOrdersEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minPaidEventOrders"
                            className="text-sm"
                          >
                            Min paid event orders
                          </label>
                        </div>
                        {eligibilityBuilder.minPaidEventOrdersEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidEventOrders: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidCustomOrdersEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minPaidCustomOrders"
                            className="text-sm"
                          >
                            Min paid custom orders
                          </label>
                        </div>
                        {eligibilityBuilder.minPaidCustomOrdersEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minPaidCustomOrders: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minTotalRevenueMinorEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-minTotalRevenueMinor"
                            className="text-sm"
                          >
                            Minimum total revenue (minor)
                          </label>
                        </div>
                        {eligibilityBuilder.minTotalRevenueMinorEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                minTotalRevenueMinor: e.target.value,
                              }))
                            }
                          />
                        )}
                      ...
                      <div className="flex items-center gap-3">
                        <Checkbox
                          ...
                          checked={
                            eligibilityBuilder.registeredWithinCampaignWindow
                          }
                          onCheckedChange={(checked) =>
                            setEligibilityBuilder((prev) => ({
                              ...prev,
                              registeredWithinCampaignWindow: checked === true,
                            }))
                          }
                        />
                        <label
                          htmlFor="rule-registeredWithinCampaignWindow"
                          className="text-sm"
                        >
                          Registered within campaign window
                        </label>
                      </div>
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                vendorCategoryIdsEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-vendorCategoryIds"
                            className="text-sm"
                          >
                            Vendor category IDs
                          </label>
                        ...
                        {eligibilityBuilder.vendorCategoryIdsEnabled && (
                          <Input
                            ...
                            onChange={(e) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                vendorCategoryIds: e.target.value,
                              }))
                            }
                          />
                        )}
                        ...
                        <div className="flex items-center gap-3">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setEligibilityBuilder((prev) => ({
                                ...prev,
                                vendorCitiesEnabled: checked === true,
                              }))
                            }
                          />
                          <label
                            htmlFor="rule-vendorCities"
                            className="text-sm"
                          >
                            Vendor cities
                          </label>
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() =>
                deletingCampaignId && deleteMutation.mutate(deletingCampaignId)
              }
