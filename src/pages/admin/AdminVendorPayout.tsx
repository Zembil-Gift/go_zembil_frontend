...
export default function AdminVendorPayout() {
  ...
  const parsedVendorId = vendorIdInput.trim()
    ? Number(vendorIdInput.trim())
    : undefined;
  ...
  const restartPayoutMutation = useMutation({
    mutationFn: (payoutId: number) =>
      adminService.restartVendorPayout(payoutId),
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error?.message || "Unable to restart payout. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const currentPageNumber =
    payouts?.pageable?.pageNumber ?? payouts?.number ?? payoutPage;
        ...
        <Card>
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>
                Pulled from /api/admin/payouts with optional status and vendor
                filters.
              </CardDescription>
            </div>
            <Button
              ...
              <>
                        ...
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            {item.trackedPlatformFee
                              ? formatMoney(
                                  item.feeTrackingAmount,
                                  item.feeTrackingCurrency
                                )
                              : "-"}
                          </TableCell>
                          <TableCell>
                            {item.feeTrackingExchangeRate
                              ? item.feeTrackingExchangeRate.toLocaleString(
                                  undefined,
                          ...
                          <TableCell>
                            {item.status?.toUpperCase() === "FAILED" ? (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  restartPayoutMutation.mutate(item.id)
                                }
                                disabled={
                ...
                <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-muted-foreground">
                    ...
                    {(payouts?.totalElements ?? 0).toLocaleString()} total
                    records)
                  </p>
                    ...
                    <Button
                      ...
                      disabled={
                        (currentPageNumber ?? 0) + 1 >=
                          Math.max(totalPages, 1) || isFetching
                      }
                    >
                      Next
                    </Button>
